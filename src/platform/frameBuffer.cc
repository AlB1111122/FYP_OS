...
FrameBuffer::FrameBuffer(bool doubleBufferd) {
  ...
  if (FB_mailbox.writeRead(REQ_CHANNEL)) {
    ...
    this->baseFb = reinterpret_cast<unsigned char *>(
        static_cast<int64_t>(mbox->fbPointer));
    this->fb = this->baseFb;
  }
}

uint32_t FrameBuffer::getPitch() { return this->pitch; }

uint32_t FrameBuffer::getHeight() { return (this->height); }

void FrameBuffer::drawPixel(int32_t x, int32_t y, unsigned char attr) {
  int32_t offs = getXYOffset(x, y);
  *((uint32_t *)(this->fb + offs)) = vgapal[attr & 0x0f];
}

void FrameBuffer::drawPixelRGB(int32_t x, int32_t y, uint32_t colourRGB) {
  int32_t offs = getXYOffset(x, y);
  *((uint32_t *)(this->fb + offs)) = colourRGB;
}

void FrameBuffer::drawByLine(uint8_t *buffer, int32_t xSz, int32_t ySz) {
  for (int32_t i = 0; i < ySz; i++) {
    uint32_t offs = i * this->pitch;
    memcpy((this->fb + offs), (buffer + (i * xSz)), xSz);
  }
}

void FrameBuffer::bufferCpy(uint8_t *buffer) {
  int64_t unsigned fbSize = this->width * this->height * 4;
  memcpy(this->fb, buffer, fbSize);
}

void FrameBuffer::drawChar(unsigned char ch, int32_t x, int32_t y,
                           unsigned char attr) {
  unsigned char *glyph =
...
void FrameBuffer::drawString(int32_t x, int32_t y, char *s,
                             unsigned char attr) {
  while (*s) {
    if (*s == '\r') {
      x = 0;
...
int32_t FrameBuffer::getXYOffset(int32_t x, int32_t y) {
  return (y * this->pitch) + (x * 4);
}

// expects rgb
void FrameBuffer::pixelByPixelDraw(int32_t xSrc, int32_t ySrc, uint8_t *src) {
  int32_t i = 1;
  for (int32_t y = 0; y < ySrc; y++) {
    for (int32_t x = 0; x < xSrc; x++) {
...
void FrameBuffer::swapFb() {
  ...
  int32_t newOffset = (this->height / 2) * this->secondBuffer;
